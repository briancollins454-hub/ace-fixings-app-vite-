/**
 * Netlify Function: submitVatVerification
 * 
...
exports.handler = async (event) => {
  ...
  try {
    const { customerEmail, businessName, country, vatNumber } = JSON.parse(event.body);
    ...
    if (!adminToken) {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ error: "Missing admin API token" }),
      };
    }
    ...
    if (!searchResponse?.customers?.edges?.length) {
      ...
      return {
        statusCode: 404,
        body: JSON.stringify({ error: `Customer with email ${customerEmail} not found` }),
      };
    }
